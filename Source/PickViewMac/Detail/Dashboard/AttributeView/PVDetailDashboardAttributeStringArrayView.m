//
//  PVDetailDashboardAttributeStringArrayView.m
//  PickViewMac
//
...
@interface PVDetailDashboardAttributeStringArrayView ()

@property(nonatomic, copy) NSArray<PVDetailLabel *> *labels;
@property(nonatomic, copy) NSArray<CALayer *> *sepLayers;
@property(nonatomic, strong) NSButton *danceButton;

@end

@implementation PVDetailDashboardAttributeStringArrayView {
...
- (void)layout {
    [super layout];
    [self.labels enumerateObjectsUsingBlock:^(PVDetailLabel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        PVDetailLabel *prevLabel = (idx > 0 ? self.labels[idx - 1] : nil);
        $(obj).fullFrame.heightToFit.y(prevLabel ? (prevLabel.$maxY + self -> _labelsVerInterSpace) : 0);
        
        if (idx > 0) {
            if ([self.sepLayers pv_inspect_hasIndex:(idx - 1)]) {
...
- (NSSize)sizeThatFits:(NSSize)limitedSize {
    limitedSize.height = [self.labels pv_inspect_reduceCGFloat:^CGFloat(CGFloat accumulator, NSUInteger idx, PVDetailLabel *obj) {
...
- (void)renderWithAttribute {
    ...
    self.labels = [self.labels pv_inspect_resizeWithCount:lists.count add:^PVDetailLabel *(NSUInteger idx) {
    ...
    } remove:^(NSUInteger idx, PVDetailLabel *label) {
        [label removeFromSuperview];
        
    } doNext:^(NSUInteger idx, PVDetailLabel *label) {
    ...
    if (lists.count > 1) {
        self.sepLayers = [self.sepLayers pv_inspect_resizeWithCount:(lists.count - 1) add:^CALayer *(NSUInteger idx) {
            CALayer *layer = [CALayer new];
            [layer pv_inspect_removeImplicitAnimations];
    ...
    if ([self.attribute.identifier isEqualToString:PVAttr_Class_Class_Class]) {
