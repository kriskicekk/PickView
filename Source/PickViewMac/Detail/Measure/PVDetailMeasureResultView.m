...
#define addHor(arg_array, arg_startX, arg_endX, arg_y, arg_value) [arg_array addObject:[PVDetailMeasureResultHorLineData dataWithStartX:arg_startX endX:arg_endX y:arg_y value:arg_value]]
#define addVer(arg_array, arg_startY, arg_endY, arg_x, arg_value) [arg_array addObject:[PVDetailMeasureResultVerLineData dataWithStartY:arg_startY endY:arg_endY x:arg_x value:arg_value]]
...
@interface PVDetailMeasureResultView ()

/// mainLayer 指 selectedLayer，referLayer 指 hovered layer
@property(nonatomic, strong) PVDetailBaseView *contentView;
...
@property(nonatomic, strong) PVDetailBaseView *linesContainerView;
...
@property(nonatomic, strong) NSMutableArray<PVDetailTextFieldView *> *textFieldViews;
...
@implementation PVDetailMeasureResultView {
    CGFloat _horInset;
    CGFloat _verInset;
...
- (instancetype)initWithFrame:(NSRect)frameRect {
    if (self = [super initWithFrame:frameRect]) {
        ...
        self.contentView = [PVDetailBaseView new];
        // label 经常会超出范围
        ...
        self.linesContainerView = [PVDetailBaseView new];
        ...
        [self.mainImageViewBorderLayer pv_inspect_removeImplicitAnimations];
        ...
        [self.referImageViewBorderLayer pv_inspect_removeImplicitAnimations];
        ...
        [self.horSolidLinesLayer pv_inspect_removeImplicitAnimations];
        self.horSolidLinesLayer.strokeColor = PVColorMake(10, 127, 251).CGColor;
        ...
        [self.verSolidLinesLayer pv_inspect_removeImplicitAnimations];
        self.verSolidLinesLayer.strokeColor = PVColorMake(209, 120, 0).CGColor;
...
- (void)updateColors {
    [super updateColors];
    NSColor *borderColor = self.isDarkMode ? PVColorMake(123, 123, 123) : PVColorMake(190, 190, 190);
...
- (void)renderWithMainRect:(CGRect)originalMainRect mainImage:(PVImage *)mainImage referRect:(CGRect)originalReferRect referImage:(PVImage *)referImage {
...
- (void)_renderLinesAndLabels {
    ...
    /** 水平方向的线 START **/
    NSMutableArray<PVDetailMeasureResultHorLineData *> *horDatas = [NSMutableArray array];
    ...
    /** 竖直方向的线 START **/
    NSMutableArray<PVDetailMeasureResultVerLineData *> *verDatas = [NSMutableArray array];
    ...
    [horDatas enumerateObjectsUsingBlock:^(PVDetailMeasureResultHorLineData * _Nonnull data, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        PVDetailTextFieldView *labelView = [self _dequeueAvailableTextField];
        labelView.backgroundColor = NSColor.systemBlueColor;
        labelView.textField.stringValue = [NSString pv_inspect_stringFromDouble:data.displayValue decimal:2];
        $(labelView).sizeToFit.height(_labelHeight).midX(data.startX + (data.endX - data.startX) / 2.0).maxY(data.y - 5);
    }];
    [verDatas enumerateObjectsUsingBlock:^(PVDetailMeasureResultVerLineData * _Nonnull data, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        PVDetailTextFieldView *labelView = [self _dequeueAvailableTextField];
        labelView.backgroundColor = PVColorRGBAMake(209, 120, 0, 1.0);
        labelView.textField.stringValue = [NSString pv_inspect_stringFromDouble:data.displayValue decimal:2];
        ...
        if ([self checkOverlapOfTargetLabelView:labelView]) {
            ...
            labelView.backgroundColor = PVColorRGBAMake(209, 120, 0, 0.5);
        }
    }];
...
- (CGFloat)_calculateScaleFactorWithMainRect:(CGRect)mainRect referRect:(CGRect)referRect {
    ...
    CGFloat maxContentHeight = MAX((self.window.frame.size.height - [PVDetailNavigationManager sharedInstance].windowTitleBarHeight) * .8, 200);
...
- (void)_hideAllLabels {
    [self.textFieldViews enumerateObjectsUsingBlock:^(PVDetailTextFieldView * _Nonnull view, NSUInteger idx, BOOL * _Nonnull stop) {
        view.hidden = YES;
    }];
}

- (PVDetailTextFieldView *)_dequeueAvailableTextField {
    if (!self.textFieldViews) {
        self.textFieldViews = [NSMutableArray array];
    }
    PVDetailTextFieldView *resultView = [self.textFieldViews pv_inspect_firstFiltered:^BOOL(PVDetailTextFieldView *view) {
        return view.hidden;
    }];
    if (!resultView) {
        resultView = [PVDetailTextFieldView labelView];
