//
//  PVDetailHierarchyHandlersPopoverItemView.m
//  PickViewMac
//
...
@interface PVDetailHierarchyHandlersPopoverItemView ()

@property(nonatomic, strong) PVEventHandler *eventHandler;
...
@property(nonatomic, strong) PVDetailTextsMenuView *contentView;
...
@implementation PVDetailHierarchyHandlersPopoverItemView {
    CGFloat _contentX;
    CGFloat _insetRight;
    CGFloat _verInset;
...
- (instancetype)initWithEventHandler:(PVEventHandler *)eventHandler editable:(BOOL)editable {
    if (self = [self initWithFrame:NSZeroRect]) {
        ...
        self.contentView = [PVDetailTextsMenuView new];
        self.contentView.font = NSFontMake(13);
        [self addSubview:self.contentView];
        
        self.topSepLayer.backgroundColor = self.isDarkMode ? PVColorRGBAMake(255, 255, 255, .15).CGColor : PVColorRGBAMake(0, 0, 0, .12).CGColor;
        
        NSMutableArray<PVStringTwoTuple *> *texts = [NSMutableArray array];
        if (eventHandler.handlerType == PVEventHandlerTypeGesture) {
            [texts addObject:[PVStringTwoTuple tupleWithFirst:@"Enabled" second:@""]];
            ...
            if (editable) {
                ...
                [texts addObject:[PVStringTwoTuple tupleWithFirst:@"Enabled" second:(eventHandler.gestureRecognizerIsEnabled ? @"YES" : @"NO")]];
            }
            
            [texts addObject:[PVStringTwoTuple tupleWithFirst:@"Delegate" second:(eventHandler.gestureRecognizerDelegator ? : @"nil")]];
        ...
        if (eventHandler.targetActions.count == 0) {
            [texts addObject:[PVStringTwoTuple tupleWithFirst:@"Target" second:@"nil"]];
            [texts addObject:[PVStringTwoTuple tupleWithFirst:@"Action" second:@"NULL"]];
        } else if (eventHandler.targetActions.count == 1) {
            PVStringTwoTuple *tuple = eventHandler.targetActions.firstObject;
            [texts addObject:[PVStringTwoTuple tupleWithFirst:@"Target" second:tuple.first]];
            [texts addObject:[PVStringTwoTuple tupleWithFirst:@"Action" second:tuple.second]];
        } else {
            [eventHandler.targetActions enumerateObjectsUsingBlock:^(PVStringTwoTuple * _Nonnull tuple, NSUInteger idx, BOOL * _Nonnull stop) {
                [texts addObject:[PVStringTwoTuple tupleWithFirst:[NSString stringWithFormat:@"Target %@", @(idx + 1)] second:tuple.first]];
                [texts addObject:[PVStringTwoTuple tupleWithFirst:[NSString stringWithFormat:@"Action %@", @(idx + 1)] second:tuple.second]];
            }];
        }
...
- (void)_handleGestureButton:(NSButton *)button {
    NSWindow *mainWindow = [PVDetailNavigationManager sharedInstance].staticWindowController.window;
    
    if (!InspectingApp) {
        AlertError(PVInspectErr_NoConnect, mainWindow);
        [self _renderRecognizerEnabledButton];
        return;
    }
