...
@class PVHierarchyInfo, PVDisplayItem, PVDetailPreferenceManager;

typedef NS_ENUM(NSUInteger, PVDetailHierarchyDataSourceState) {
    PVDetailHierarchyDataSourceStateNormal,
    PVDetailHierarchyDataSourceStateSearch,
    PVDetailHierarchyDataSourceStateFocus,
};

@interface PVDetailHierarchyDataSource : NSObject

/// 业务可以 observe 该属性
@property(nonatomic, assign, readonly) PVDetailHierarchyDataSourceState state;
...
@property(nonatomic, copy) NSArray<PVDisplayItem *> *rawFlatItems;

/// 一维数组，包含所有 hierarchy 树中可见和不可见的 displayItems
/// 搜索或聚焦状态下，flatItems 是 rawFlatItems 的子集（normal 状态下，flatItems 和 rawFlatItems 等价）
@property(nonatomic, copy) NSArray<PVDisplayItem *> *flatItems;

/// 一维数组，只包括在 hierarchy 树中因为未被折叠而可见的 displayItems
/// 业务可以 observe 该属性
@property(nonatomic, copy, readonly) NSArray<PVDisplayItem *> *displayingFlatItems;
...
/// 当前选中的 item
@property(nonatomic, weak) PVDisplayItem *selectedItem;

/// 当前被鼠标 hover 的 item
@property(nonatomic, weak) PVDisplayItem *hoveredItem;
...
@property(nonatomic, strong, readonly) PVHierarchyInfo *rawHierarchyInfo;
