...
@interface PVDetailHierarchyView () <PVDetailTableViewDelegate, PVDetailTableViewDataSource, NSMenuDelegate, NSTextFieldDelegate>
...
@property(nonatomic, strong) PVDetailTextFieldView *searchTextFieldView;

@property(nonatomic, strong) PVDetailLabel *emptyDataLabel;
...
@implementation PVDetailHierarchyView
...
- (instancetype)initWithDataSource:(PVDetailHierarchyDataSource *)dataSource {
    if (self = [super initWithFrame:CGRectZero]) {
        ...
        [self.guidesShapeLayer pv_inspect_removeImplicitAnimations];
        ...
        self.searchTextFieldView = [PVDetailTextFieldView new];
        ...
        self.searchTextFieldView.borderPosition = PVDetailViewBorderPositionTop;
        self.searchTextFieldView.borderColors = PVDetailColorsCombine(PVColorMake(200, 201, 202), PVColorMake(67, 68, 69));
        ...
        [[RACObserve(self.tableView, contentInsets) distinctUntilChanged] subscribeNext:^(NSValue *x) {
            ...
            [PVDetailNavigationManager sharedInstance].windowTitleBarHeight = insetTop;
        }];
        
        [RACObserve(dataSource, selectedItem) subscribeNext:^(PVDisplayItem * _Nullable item) {
            @strongify(self);
            [self.tableView reloadDataWithOffset];
            [self scrollToMakeItemVisible:item];
        }];
        
        [[RACObserve(self.dataSource, hoveredItem) distinctUntilChanged] subscribeNext:^(PVDisplayItem * _Nullable x) {
            @strongify(self);
            [self updateGuidesWithHoveredItem:x];
        }];
        
        [RACObserve(dataSource, displayingFlatItems) subscribeNext:^(NSArray<PVDisplayItem *> *x) {
            @strongify(self);
...
- (void)renderWithDisplayItems:(NSArray<PVDisplayItem *> *)displayItems {
    self.displayItems = displayItems;
    
    _minIndentLevel = [[displayItems pv_inspect_reduce:^NSNumber *(NSNumber *accumulator, NSUInteger idx, PVDisplayItem *obj) {
    ...
    if (displayItems.count == 0) {
        if (!self.emptyDataLabel) {
            self.emptyDataLabel = [PVDetailLabel new];
...
- (void)updateColors {
    [super updateColors];
    self.guidesShapeLayer.strokeColor = self.isDarkMode ? PVColorRGBAMake(255, 255, 255, .3).CGColor : PVColorRGBAMake(0, 0, 0, .3).CGColor;
}
...
- (void)tableView:(PVDetailTableView *)tableView didHoverAtRow:(NSInteger)row {
    // 注意这里 item 可能为 nil
    PVDisplayItem *item = [self _safeItemAtRow:row];
...
- (PVDetailTableRowView *)tableView:(NSTableView *)tableView rowViewForRow:(NSInteger)row {
    PVDisplayItem *item = [self _safeItemAtRow:row];
    if (!item) {
        NSAssert(NO, @"");
    ...
    PVDetailHierarchyRowView *view = [tableView makeViewWithIdentifier:@"cell" owner:self];
    if (!view) {
        view = [[PVDetailHierarchyRowView alloc] initWithDataSource:self.dataSource];
        ...
        [view.menu pv_inspect_bindObjectWeakly:view forKey:kMenuBindKey_RowView];
...
- (void)menuNeedsUpdate:(NSMenu *)menu {
    PVDetailHierarchyRowView *rowView = [menu pv_inspect_getBindObjectForKey:kMenuBindKey_RowView];
    PVDisplayItem *displayItem = rowView.displayItem;
    ...
    if (displayItem.displayingObject.ivarTraces.count) {
        NSArray<NSString *> *ivarNames = [[displayItem.displayingObject.ivarTraces pv_inspect_map:^id(NSUInteger idx, PVIvarTrace *value) {
            NSString *name = value.ivarName;
            if ([name hasPrefix:@"_"]) {
                name = [name substringFromIndex:1];
            }
            return name;
        }] pv_inspect_nonredundantArray];
        if (ivarNames.count) {
    ...
    [stringsToCopy enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (obj.length == 0) {
            NSAssert(NO, @"PVDetailHierarchyView, menuNeedsUpdate, stringsToCopy length is zero.");
            return;
        }
        [menu addItem:({
            NSMenuItem *item = [NSMenuItem new];
    ...
    if ([displayItem hasPreviewBoxAbility]) {
        if (displayItem.inNoPreviewHierarchy) {
            [menu addItem:({
                ...
                if (displayItem.doNotFetchScreenshotReason == PVFetchScreenshotPermitted) {
...
- (void)_handlePrintItem:(NSMenuItem *)menuItem {
    PVDetailHierarchyRowView *view = [menuItem.menu pv_inspect_getBindObjectForKey:kMenuBindKey_RowView];
    PVDisplayItem *item = view.displayItem;
...
- (void)_handleExportScreenshot:(NSMenuItem *)menuItem {
    PVDetailHierarchyRowView *view = [menuItem.menu pv_inspect_getBindObjectForKey:kMenuBindKey_RowView];
    [PVDetailExportManager exportScreenshotWithDisplayItem:view.displayItem];
}
...
- (void)updateGuidesWithHoveredItem:(PVDisplayItem *)item {
    ...
    PVDisplayItem *rootItem = item.superItem;
    NSUInteger rootRow = [self.displayItems indexOfObject:rootItem];
    NSArray<PVDisplayItem *> *childrenItems = [rootItem.subitems pv_inspect_filter:^BOOL(PVDisplayItem *obj) {
    ...
    CGFloat rootX = [PVDetailHierarchyRowView dislosureMidXWithIndentLevel:rootItem.indentLevel - self.minIndentLevel];
    ...
    [childrenItems enumerateObjectsUsingBlock:^(PVDisplayItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (PVDisplayItem *)_safeItemAtRow:(NSInteger)row {
    return [self.displayItems pv_inspect_hasIndex:row] ? self.displayItems[row] : nil;
}

- (PVDetailStaticHierarchyDataSource *)realtimeDataSource {
    if (![self.dataSource isKindOfClass:[PVDetailStaticHierarchyDataSource class]]) {
        NSAssert(NO, @"");
        return nil;
    }
    return (PVDetailStaticHierarchyDataSource *)self.dataSource;
}
