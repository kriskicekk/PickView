...
//  PickView 项目鸣谢：https://qxh1ndiez2w.feishu.cn/docx/YIFjdE4gIolp3hxn1tGckiBxnWf

#import "PVDetailPreviewView.h"
#import "PVDetailDisplayItemNode.h"
#import "PVDisplayItem.h"
#import "PVDetailHierarchyDataSource.h"

const CGFloat PVPreviewMinScale = 0;
const CGFloat PVPreviewMaxScale = 1;

const CGFloat PVPreviewMinZInterspace = 0;
const CGFloat PVPreviewMaxZInterspace = 1;

@interface PVDetailPreviewView ()
...
@property(nonatomic, copy) NSArray<PVDisplayItem *> *flatDisplayItems;
@property(nonatomic, strong) NSMutableArray<PVDetailDisplayItemNode *> *displayItemNodes;

@end

@implementation PVDetailPreviewView
...
- (instancetype)initWithDataSource:(PVDetailHierarchyDataSource *)dataSource {
    if (self = [super initWithFrame:CGRectZero options:nil]) {
        ...
        rightLight.categoryBitMask = PVPreviewBitMask_HasLight;
        ...
        leftLight.categoryBitMask = PVPreviewBitMask_HasLight;
...
- (void)setAppScreenSize:(CGSize)appScreenSize {
    ...
    [self.displayItemNodes enumerateObjectsUsingBlock:^(PVDetailDisplayItemNode * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        obj.screenSize = appScreenSize;
    }];
}

- (void)renderWithDisplayItems:(NSArray<PVDisplayItem *> *)items discardCache:(BOOL)discardCache {
    NSLog(@"PVDetailPreviewView - render %@ items", @(items.count));
    
    self.flatDisplayItems = items;
    
    NSMutableArray<PVDetailDisplayItemNode *> *nodesToBeDiscarded = nil;
    if (discardCache) {
        nodesToBeDiscarded = [NSMutableArray array];
    }
    
    [self.displayItemNodes pv_inspect_dequeueWithCount:items.count add:^PVDetailDisplayItemNode *(NSUInteger idx) {
        PVDetailDisplayItemNode *newNode = [[PVDetailDisplayItemNode alloc] initWithDataSource:self.dataSource];
    ...
    } notDequeued:^(NSUInteger idx, PVDetailDisplayItemNode *node) {
        [node removeFromParentNode];
        if (discardCache) {
            [nodesToBeDiscarded addObject:node];
        }
        
    } doNext:^(NSUInteger idx, PVDetailDisplayItemNode *node) {
        if (!node.parentNode) {
            [self.stageNode addChildNode:node];
        }

        PVDisplayItem *displayItem = items[idx];
...
- (void)updateZPosition {
    [self.flatDisplayItems enumerateObjectsUsingBlock:^(PVDisplayItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)_updateZPositionByZIndex {
    CGFloat interspace;
    if (self.dimension == PVPreviewDimension2D) {
        interspace = 0.01;
    } else {
    ...
    [self.displayItemNodes enumerateObjectsUsingBlock:^(PVDetailDisplayItemNode * _Nonnull node, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    [self.displayItemNodes enumerateObjectsUsingBlock:^(PVDetailDisplayItemNode * _Nonnull node, NSUInteger idx, BOOL * _Nonnull stop) {
        PVDisplayItem *item = node.displayItem;
...
- (void)_updateZIndexForItem:(PVDisplayItem *)item {
    item.previewZIndex = -1;
    if (item.displayingInHierarchy) {
        PVDisplayItem *referenceItem = [self _maxZIndexForOverlappedItemUnderItem:item];
        if (referenceItem) {
 ...
 */
- (PVDisplayItem *)_maxZIndexForOverlappedItemUnderItem:(PVDisplayItem *)item {
    ...
    __block PVDisplayItem *targetItem = nil;
    [self.flatDisplayItems enumerateObjectsAtIndexes:indexesBelow options:NSEnumerationReverse usingBlock:^(PVDisplayItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)didSelectItem:(PVDisplayItem *)item {
    ...
    PVDetailDisplayItemNode *displayItemNode = item.previewNode;
...
- (PVDisplayItem *)displayItemAtPoint:(CGPoint)point {
    NSArray *hitResults = [self hitTest:point options:@{SCNHitTestOptionCategoryBitMask:@(PVPreviewBitMask_Selectable),
                                                        SCNHitTestOptionSearchMode: @(SCNHitTestSearchModeClosest),
                                                        SCNHitTestIgnoreHiddenNodesKey:@(NO)}];
    if ([hitResults count] > 0) {
        SCNHitTestResult *result = [hitResults objectAtIndex:0];
        PVDetailDisplayItemNode *targetNode = (PVDetailDisplayItemNode *)result.node.parentNode;
        NSAssert([targetNode isKindOfClass:[PVDetailDisplayItemNode class]], @"");
...
- (void)setZInterspace:(CGFloat)zInterspace {
    _zInterspace = MIN(MAX(zInterspace, PVPreviewMinZInterspace), PVPreviewMaxZInterspace);
    [self _updateZPositionByZIndex];
}
...
- (void)setDimension:(PVPreviewDimension)dimension animated:(BOOL)animated {
    ...
    if (dimension == PVPreviewDimension3D) {
        // 3D
    } else {
        // 2D
...
- (void)setIsDarkMode:(BOOL)isDarkMode {
    ...
    self.backgroundColor = isDarkMode ? PVColorMake(19, 20, 21) : PVColorMake(249, 249, 249);
