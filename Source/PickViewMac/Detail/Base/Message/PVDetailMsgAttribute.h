...
@interface PVDetailMsgActionParams : NSObject
...
@interface PVDetailMsgAttribute : NSObject
...
/**
 ...
 调用该方法后，所有此前通过 subscribe:action: 相关方法注册的 subscriber 的 action 都会被调用，参数是一个 PVDetailMsgActionParams 对象
 如果传入了 ignoreSubscriber，则 ignoreSubscriber 这个对象不会收到这次通知
 如果传入的 value 和之前已有的 value 是 equal 的，则该方法不会产生任何效果
 传入的 userInfo 对象可在 PVDetailMsgActionParams 中被读取
 */
- (void)setValue:(id)value ignoreSubscriber:(id)ignoreSubscriber userInfo:(id)userInfo;

/// target, relatedObject 均不会被强引用，action 方法的参数是一个 PVDetailMsgActionParams
...
@interface PVDetailDoubleMsgAttribute : PVDetailMsgAttribute
...
@interface PVDetailIntegerMsgAttribute : PVDetailMsgAttribute
...
@interface PVDetailBOOLMsgAttribute : PVDetailMsgAttribute
