...
#import "PVDetailInputSearchSuggestionWindowController.h"
#import "PVDetailInputSearchSuggestionsContentView.h"
#import "PVDetailInputSearchSuggestionItem.h"

@interface PVDetailInputSearchView () <NSTextFieldDelegate>

@property(nonatomic, strong) PVDetailTextFieldView *textFieldView;
@property(nonatomic, strong) PVDetailInputSearchSuggestionWindowController *suggestionWc;

@property(nonatomic, copy) NSString *previousInput;

@end

@implementation PVDetailInputSearchView

- (instancetype)initWithThrottleTime:(CGFloat)throttleTime {
    if (self = [self initWithFrame:NSZeroRect]) {
        self.textFieldView = [PVDetailTextFieldView new];
        ...
        self.suggestionWc = [PVDetailInputSearchSuggestionWindowController new];
        ...
        [[self.textFieldView.textField.rac_textSignal throttle:throttleTime] subscribeNext:^(NSString * _Nullable x) {
            @strongify(self);
            NSArray<PVDetailInputSearchSuggestionItem *> *items;
...
- (void)_handleClickTableView {
    PVDetailInputSearchSuggestionItem *item = [self.suggestionWc.suggestionsView currentSelectedItem];
    if (item.text) {
