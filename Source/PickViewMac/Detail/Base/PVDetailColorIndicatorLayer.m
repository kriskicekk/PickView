...
@interface PVDetailColorIndicatorLayer ()
...
@implementation PVDetailColorIndicatorLayer

- (instancetype)init {
    if (self = [super init]) {
        [self pv_inspect_removeImplicitAnimations];
        
        self.borderWidth = 1;
        
        _color = PVColorMake(0, 0, 0);
        self.colorLayer = [CALayer layer];
        self.colorLayer.backgroundColor = self.color.CGColor;
        [self.colorLayer pv_inspect_removeImplicitAnimations];
...
- (NSColor *)_contrastColorForColor:(NSColor *)color {
    if (!color) {
        return PVColorMake(191, 191, 191);
    }
...
- (void)_createImageLayerIfNeeded {
    ...
    [self.imageLayer pv_inspect_removeImplicitAnimations];
...
+ (NSImage *)imageWithColor:(NSColor *)color shapeSize:(NSSize)shapeSize insets:(NSEdgeInsets)insets {
    static PVDetailColorIndicatorLayer *layer = nil;
    if (!layer) {
        layer = [PVDetailColorIndicatorLayer new];
    }
