...
@interface PVDetailTableView () <NSTableViewDelegate, NSTableViewDataSource>

@property(nonatomic, strong) PVDetailTableViewHorizontalScrollWidthManager *horizontalScrollWidthManager;
...
@implementation PVDetailTableView
...
- (void)reloadData {
    ...
    if (self.canScrollHorizontally) {
        self.horizontalScrollWidthManager = [PVDetailTableViewHorizontalScrollWidthManager new];
        @weakify(self);
...
- (PVDetailTableRowView *)makeViewWithIdentifier:(NSString *)identifier owner:(id)owner {
...
- (NSTableRowView *)tableView:(NSTableView *)tableView rowViewForRow:(NSInteger)row {
    if ([self.delegate respondsToSelector:@selector(tableView:rowViewForRow:)]) {
        ...
        if ([rowView isKindOfClass:[PVDetailTableRowView class]]) {
            ((PVDetailTableRowView *)rowView).isHovered = (self.hoveredRow == row);
            if (self.adjustsSelectionAutomatically) {
                ((PVDetailTableRowView *)rowView).isSelected = (self.selectedRow == row);
            }
            
            if (self.canScrollHorizontally) {
                ((PVDetailTableRowView *)rowView).horizontalScrollWidthManager = self.horizontalScrollWidthManager;
            }
        }
        return rowView;
    }
...
- (void)setHoveredRow:(NSInteger)hoveredRow {
    ...
    if (prevHoveredRow >= 0 && prevHoveredRow < self.tableView.numberOfRows) {
        ...
        if ([rowView isKindOfClass:[PVDetailTableRowView class]]) {
            ((PVDetailTableRowView *)rowView).isHovered = NO;
        }
    }
    ...
    if (hoveredRow >= 0 && hoveredRow < self.tableView.numberOfRows) {
        ...
        if (canHover) {
            ...
            if ([rowView isKindOfClass:[PVDetailTableRowView class]]) {
                ((PVDetailTableRowView *)rowView).isHovered = YES;
            }
        }
    }
...
- (void)setSelectedRow:(NSInteger)selectedRow {
    ...
    if (prevSelectedRow >= 0 && prevSelectedRow < self.tableView.numberOfRows) {
        ...
        if ([rowView isKindOfClass:[PVDetailTableRowView class]]) {
            ((PVDetailTableRowView *)rowView).isSelected = NO;
        }
    }
    ...
    if (selectedRow >= 0 && selectedRow < self.tableView.numberOfRows) {
        ...
        if ([rowView isKindOfClass:[PVDetailTableRowView class]]) {
            ((PVDetailTableRowView *)rowView).isSelected = YES;
        }
        if ([self.delegate respondsToSelector:@selector(tableView:didSelectRow:)]) {
            [self.delegate tableView:self didSelectRow:selectedRow];
        }
