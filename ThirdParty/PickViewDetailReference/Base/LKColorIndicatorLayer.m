...
- (instancetype)init {
    if (self = [super init]) {
        [self pickview_removeImplicitAnimations];
        
        self.borderWidth = 1;
        
        _color = PickViewColorMake(0, 0, 0);
        self.colorLayer = [CALayer layer];
        self.colorLayer.backgroundColor = self.color.CGColor;
        [self.colorLayer pickview_removeImplicitAnimations];
...
- (NSColor *)_contrastColorForColor:(NSColor *)color {
    if (!color) {
        return PickViewColorMake(191, 191, 191);
    }
...
- (void)_createImageLayerIfNeeded {
    ...
    [self.imageLayer pickview_removeImplicitAnimations];
