//
//  ShortCocoa+PickViewClient.m
//  PickView
//
//  Created by kris cheng on 2026/7/9.
//

#import "ShortCocoa+PickViewClient.h"
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        Method newMethod = class_getInstanceMethod([self class], @selector(pickview_sizeToFit));
        method_exchangeImplementations(oriMethod, newMethod);
        
        Method oriMethod1 = class_getInstanceMethod([self class], @selector(heightToFit));
        Method newMethod1 = class_getInstanceMethod([self class], @selector(pickview_heightToFit));
        method_exchangeImplementations(oriMethod1, newMethod1);
    });
}

- (ShortCocoa *)pickview_sizeToFit {
...
- (ShortCocoa *)pickview_heightToFit {
    [self unpackClassA:[LKBaseView class] doA:^(LKBaseView * _Nonnull view, BOOL * _Nonnull stop) {
