//
//  PickViewDisplayItem+PickViewClient.m
//  PickView
//
...
@implementation PickViewDisplayItem (PickViewClient)
...
- (NSString *)subtitle {
    ...
    PickViewObject *representedObject = self.viewObject ? : self.layerObject;
    if (representedObject.specialTrace.length) {
        return representedObject.specialTrace;
        
    }
    if (representedObject.ivarTraces.count) {
        NSArray<NSString *> *ivarNameList = [representedObject.ivarTraces pickview_map:^id(NSUInteger idx, PickViewIvarTrace *value) {
...
- (void)enumerateSelfAndAncestors:(void (^)(PickViewDisplayItem *, BOOL *))block {
    if (!block) {
        return;
    }
    PickViewDisplayItem *item = self;
    while (item) {
        BOOL shouldStop = NO;
...
- (void)enumerateAncestors:(void (^)(PickViewDisplayItem *, BOOL *))block {
    [self.superItem enumerateSelfAndAncestors:block];
}

- (void)enumerateSelfAndChildren:(void (^)(PickViewDisplayItem *item))block {
    if (!block) {
        return;
    }
    
    block(self);
    [self.subitems enumerateObjectsUsingBlock:^(PickViewDisplayItem * _Nonnull subitem, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (BOOL)itemIsKindOfClassesWithNames:(NSSet<NSString *> *)targetClassNames {
    ...
    PickViewObject *selfObj = self.viewObject ? : self.layerObject;
    if (!selfObj) {
        return NO;
    }
