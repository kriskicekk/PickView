...
- (LKInputSearchSuggestionItem *)currentSelectedItem {
    ...
    return [self.items pickview_safeObjectAtIndex:selectedRow];
}

- (NSSize)bestSize {
    LKInputSearchSuggestionsRowView *rowView = [self pickview_getBindObjectForKey:@"calculatingRowView"];
    if (!rowView) {
        rowView = [LKInputSearchSuggestionsRowView new];
        [self pickview_bindObject:rowView forKey:@"calculatingRowView"];
    }
    CGFloat width = [self.items pickview_reduceCGFloat:^CGFloat(CGFloat accumulator, NSUInteger idx, LKInputSearchSuggestionItem *obj) {
...
- (NSTableRowView *)tableView:(NSTableView *)tableView rowViewForRow:(NSInteger)row {
    LKInputSearchSuggestionItem *item = [self.items pickview_safeObjectAtIndex:row];
    if (!item) {
