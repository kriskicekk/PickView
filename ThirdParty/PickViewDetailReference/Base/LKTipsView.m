...
- (instancetype)initWithFrame:(NSRect)frameRect {
    if (self = [super initWithFrame:frameRect]) {
        ...
        [self.sepLayer pickview_removeImplicitAnimations];
...
- (void)updateColors {
    [super updateColors];
    self.backgroundColor = self.isDarkMode ? PickViewColorRGBAMake(0, 0, 0, .8) : PickViewColorRGBAMake(255, 255, 255, .9);
    self.titleLabel.textColor = self.isDarkMode ? PickViewColorMake(197, 198, 199) : PickViewColorMake(108, 109, 110);
    
    NSColor *borderColor = self.isDarkMode ? PickViewColorMake(43, 44, 45) : PickViewColorMake(216, 217, 218);
...
- (void)setImageByDeviceType:(PickViewAppInfoDevice)type {
    if (type == 0) {
...
- (NSColor *)buttonTextColor {
    return self.isDarkMode ? PickViewColorMake(64, 134, 216) : PickViewColorMake(74, 145, 228);
}
...
- (void)updateColors {
    ...
    self.sepLayer.backgroundColor = PickViewColorRGBAMake(255, 255, 255, .5).CGColor;
}
...
- (void)startAnimation {
    CABasicAnimation *anim = [CABasicAnimation animationWithKeyPath:@"backgroundColor"];
    anim.fromValue = (id)PickViewColorRGBAMake(208, 2, 27, .9).CGColor;
    anim.toValue = (id)PickViewColorRGBAMake(208, 2, 27, .7).CGColor;
    anim.duration = .8;
    anim.repeatCount = HUGE_VALF;
    anim.autoreverses = YES;
    [self.layer removeAllAnimations];
