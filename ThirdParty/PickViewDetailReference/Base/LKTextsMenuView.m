...
- (void)setTexts:(NSArray<PickViewStringTwoTuple *> *)texts {
    _texts = texts.copy;
    
    [self.leftLabels pickview_dequeueWithCount:texts.count add:^LKLabel *(NSUInteger idx) {
    ...
    [self.rightLabels pickview_dequeueWithCount:texts.count add:^LKLabel *(NSUInteger idx) {
        LKLabel *label = [LKLabel new];
        label.selectable = YES;
        label.font = self.font;
        label.maximumNumberOfLines = 1;
