...
@interface LKHierarchyRowView () <PickViewDisplayItemDelegate>
...
- (void)_updateLabelStringsAndImageViewAlpha {
    ...
    if (self.displayItem.isInSearch && self.displayItem.highlightedSearchString.length) {
        // 搜索状态下，且命中了搜索词
        NSColor *bgColor = self.isDarkMode ? PickViewColorMake(190, 120, 0) : PickViewColorMake(255, 240, 100);
...
- (void)displayItem:(PickViewDisplayItem *)displayItem propertyDidChange:(PickViewDisplayItemProperty)property {
    if (property == PickViewDisplayItemProperty_IsHovered) {
        self.isHovered = (self.dataSource.hoveredItem == self.displayItem);
    } else {
        [self reRender];
    }
...
- (void)updateStrikethroughLayer {
    ...
    if (shouldShow) {
        if (!self.strikethroughLayer) {
            ...
            [self.strikethroughLayer pickview_removeImplicitAnimations];
        ...
        if (self.isSelected) {
            ...
            self.strikethroughLayer.backgroundColor = self.isDarkMode ? PickViewColorRGBAMake(255, 255, 255, .2).CGColor : PickViewColorRGBAMake(0, 0, 0, .2).CGColor;
        }
...
- (BOOL)resolveIfShouldShowStrikethrough {
    PickViewDisplayItem *item = self.displayItem;
    if (!item) {
        return NO;
    }
...
- (void)_updateEventHandlerButtonColors {
    ...
    if (self.isSelected) {
        ...
        color = PickViewColorRGBAMake(74, 144, 226, alpha);
    }
...
- (NSImage *)resolveIconImage {
    ...
    if (item.isUserCustom) {
        ...
        [item.viewObject.classChainList enumerateObjectsUsingBlock:^(NSString * _Nonnull className, NSUInteger idx, BOOL * _Nonnull stop) {
            imageName = [viewsList pickview_firstFiltered:^BOOL(NSDictionary<NSString *, NSString*> *obj) {
