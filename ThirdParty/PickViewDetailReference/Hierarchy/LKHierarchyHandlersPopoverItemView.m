...
@property(nonatomic, strong) PickViewEventHandler *eventHandler;
...
- (instancetype)initWithEventHandler:(PickViewEventHandler *)eventHandler editable:(BOOL)editable {
    if (self = [self initWithFrame:NSZeroRect]) {
        ...
        self.topSepLayer.backgroundColor = self.isDarkMode ? PickViewColorRGBAMake(255, 255, 255, .15).CGColor : PickViewColorRGBAMake(0, 0, 0, .12).CGColor;
        
        NSMutableArray<PickViewStringTwoTuple *> *texts = [NSMutableArray array];
        if (eventHandler.handlerType == PickViewEventHandlerTypeGesture) {
            [texts addObject:[PickViewStringTwoTuple tupleWithFirst:@"Enabled" second:@""]];
            ...
            if (editable) {
                ...
                [texts addObject:[PickViewStringTwoTuple tupleWithFirst:@"Enabled" second:(eventHandler.gestureRecognizerIsEnabled ? @"YES" : @"NO")]];
            }
            
            [texts addObject:[PickViewStringTwoTuple tupleWithFirst:@"Delegate" second:(eventHandler.gestureRecognizerDelegator ? : @"nil")]];
        ...
        if (eventHandler.targetActions.count == 0) {
            [texts addObject:[PickViewStringTwoTuple tupleWithFirst:@"Target" second:@"nil"]];
            [texts addObject:[PickViewStringTwoTuple tupleWithFirst:@"Action" second:@"NULL"]];
        } else if (eventHandler.targetActions.count == 1) {
            PickViewStringTwoTuple *tuple = eventHandler.targetActions.firstObject;
            [texts addObject:[PickViewStringTwoTuple tupleWithFirst:@"Target" second:tuple.first]];
            [texts addObject:[PickViewStringTwoTuple tupleWithFirst:@"Action" second:tuple.second]];
        } else {
            [eventHandler.targetActions enumerateObjectsUsingBlock:^(PickViewStringTwoTuple * _Nonnull tuple, NSUInteger idx, BOOL * _Nonnull stop) {
                [texts addObject:[PickViewStringTwoTuple tupleWithFirst:[NSString stringWithFormat:@"Target %@", @(idx + 1)] second:tuple.first]];
                [texts addObject:[PickViewStringTwoTuple tupleWithFirst:[NSString stringWithFormat:@"Action %@", @(idx + 1)] second:tuple.second]];
            }];
        }
...
- (void)_handleGestureButton:(NSButton *)button {
    ...
    if (!InspectingApp) {
        AlertError(PickViewErr_NoConnect, mainWindow);
        [self _renderRecognizerEnabledButton];
        return;
    }
