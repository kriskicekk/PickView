...
@property(nonatomic, copy) NSArray<PickViewDisplayItem *> *displayItems;
...
- (instancetype)initWithDataSource:(LKHierarchyDataSource *)dataSource {
    if (self = [super initWithFrame:CGRectZero]) {
        ...
        [self.guidesShapeLayer pickview_removeImplicitAnimations];
        ...
        self.searchTextFieldView.borderColors = LKColorsCombine(PickViewColorMake(200, 201, 202), PickViewColorMake(67, 68, 69));
        ...
        [RACObserve(dataSource, selectedItem) subscribeNext:^(PickViewDisplayItem * _Nullable item) {
            @strongify(self);
            [self.tableView reloadDataWithOffset];
            [self scrollToMakeItemVisible:item];
        }];
        
        [[RACObserve(self.dataSource, hoveredItem) distinctUntilChanged] subscribeNext:^(PickViewDisplayItem * _Nullable x) {
            @strongify(self);
            [self updateGuidesWithHoveredItem:x];
        }];
        
        [RACObserve(dataSource, displayingFlatItems) subscribeNext:^(NSArray<PickViewDisplayItem *> *x) {
            @strongify(self);
...
- (void)renderWithDisplayItems:(NSArray<PickViewDisplayItem *> *)displayItems {
    self.displayItems = displayItems;
    
    _minIndentLevel = [[displayItems pickview_reduce:^NSNumber *(NSNumber *accumulator, NSUInteger idx, PickViewDisplayItem *obj) {
...
- (void)updateColors {
    [super updateColors];
    self.guidesShapeLayer.strokeColor = self.isDarkMode ? PickViewColorRGBAMake(255, 255, 255, .3).CGColor : PickViewColorRGBAMake(0, 0, 0, .3).CGColor;
}
...
- (void)tableView:(LKTableView *)tableView didHoverAtRow:(NSInteger)row {
    // 注意这里 item 可能为 nil
    PickViewDisplayItem *item = [self _safeItemAtRow:row];
...
- (LKTableRowView *)tableView:(NSTableView *)tableView rowViewForRow:(NSInteger)row {
    ...
    if (!view) {
        ...
        [view.menu pickview_bindObjectWeakly:view forKey:kMenuBindKey_RowView];
...
- (void)menuNeedsUpdate:(NSMenu *)menu {
    LKHierarchyRowView *rowView = [menu pickview_getBindObjectForKey:kMenuBindKey_RowView];
    PickViewDisplayItem *displayItem = rowView.displayItem;
    ...
    if (displayItem.displayingObject.ivarTraces.count) {
        NSArray<NSString *> *ivarNames = [[displayItem.displayingObject.ivarTraces pickview_map:^id(NSUInteger idx, PickViewIvarTrace *value) {
            NSString *name = value.ivarName;
            if ([name hasPrefix:@"_"]) {
                name = [name substringFromIndex:1];
            }
            return name;
        }] pickview_nonredundantArray];
        if (ivarNames.count) {
    ...
    if ([displayItem hasPreviewBoxAbility]) {
        if (displayItem.inNoPreviewHierarchy) {
            [menu addItem:({
                ...
                if (displayItem.doNotFetchScreenshotReason == PickViewFetchScreenshotPermitted) {
...
- (void)_handlePrintItem:(NSMenuItem *)menuItem {
    LKHierarchyRowView *view = [menuItem.menu pickview_getBindObjectForKey:kMenuBindKey_RowView];
    PickViewDisplayItem *item = view.displayItem;
...
- (void)updateGuidesWithHoveredItem:(PickViewDisplayItem *)item {
    ...
    PickViewDisplayItem *rootItem = item.superItem;
    NSUInteger rootRow = [self.displayItems indexOfObject:rootItem];
    NSArray<PickViewDisplayItem *> *childrenItems = [rootItem.subitems pickview_filter:^BOOL(PickViewDisplayItem *obj) {
    ...
    [childrenItems enumerateObjectsUsingBlock:^(PickViewDisplayItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (PickViewDisplayItem *)_safeItemAtRow:(NSInteger)row {
    return [self.displayItems pickview_hasIndex:row] ? self.displayItems[row] : nil;
}
