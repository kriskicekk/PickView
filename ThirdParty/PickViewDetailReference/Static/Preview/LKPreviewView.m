...
const CGFloat PickViewPreviewMinScale = 0;
const CGFloat PickViewPreviewMaxScale = 1;

const CGFloat PickViewPreviewMinZInterspace = 0;
const CGFloat PickViewPreviewMaxZInterspace = 1;
...
@property(nonatomic, copy) NSArray<PickViewDisplayItem *> *flatDisplayItems;
...
- (instancetype)initWithDataSource:(LKHierarchyDataSource *)dataSource {
    if (self = [super initWithFrame:CGRectZero options:nil]) {
        ...
        rightLight.categoryBitMask = PickViewPreviewBitMask_HasLight;
        ...
        leftLight.categoryBitMask = PickViewPreviewBitMask_HasLight;
...
- (void)renderWithDisplayItems:(NSArray<PickViewDisplayItem *> *)items discardCache:(BOOL)discardCache {
    ...
    [self.displayItemNodes pickview_dequeueWithCount:items.count add:^LKDisplayItemNode *(NSUInteger idx) {
        ...
        PickViewDisplayItem *displayItem = items[idx];
...
- (void)updateZPosition {
    [self.flatDisplayItems enumerateObjectsUsingBlock:^(PickViewDisplayItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)_updateZPositionByZIndex {
    CGFloat interspace;
    if (self.dimension == PickViewPreviewDimension2D) {
        interspace = 0.01;
    } else {
    ...
    [self.displayItemNodes enumerateObjectsUsingBlock:^(LKDisplayItemNode * _Nonnull node, NSUInteger idx, BOOL * _Nonnull stop) {
        PickViewDisplayItem *item = node.displayItem;
...
- (void)_updateZIndexForItem:(PickViewDisplayItem *)item {
    item.previewZIndex = -1;
    if (item.displayingInHierarchy) {
        PickViewDisplayItem *referenceItem = [self _maxZIndexForOverlappedItemUnderItem:item];
        if (referenceItem) {
 ...
 */
- (PickViewDisplayItem *)_maxZIndexForOverlappedItemUnderItem:(PickViewDisplayItem *)item {
    ...
    __block PickViewDisplayItem *targetItem = nil;
    [self.flatDisplayItems enumerateObjectsAtIndexes:indexesBelow options:NSEnumerationReverse usingBlock:^(PickViewDisplayItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (PickViewDisplayItem *)displayItemAtPoint:(CGPoint)point {
    NSArray *hitResults = [self hitTest:point options:@{SCNHitTestOptionCategoryBitMask:@(PickViewPreviewBitMask_Selectable),
...
- (void)setZInterspace:(CGFloat)zInterspace {
    _zInterspace = MIN(MAX(zInterspace, PickViewPreviewMinZInterspace), PickViewPreviewMaxZInterspace);
    [self _updateZPositionByZIndex];
}
...
- (void)setDimension:(PickViewPreviewDimension)dimension animated:(BOOL)animated {
    ...
    if (dimension == PickViewPreviewDimension3D) {
        // 3D
    } else {
        // 2D
...
- (void)setIsDarkMode:(BOOL)isDarkMode {
    ...
    self.backgroundColor = isDarkMode ? PickViewColorMake(19, 20, 21) : PickViewColorMake(249, 249, 249);
