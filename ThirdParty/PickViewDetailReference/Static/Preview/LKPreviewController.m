...
@property(nonatomic, strong) PickViewDisplayItem *rightClickingDisplayItem;
...
- (instancetype)initWithDataSource:(LKHierarchyDataSource *)dataSource {
    if (self = [self initWithContainerView:nil]) {
        ...
        PickViewAppInfo *appInfo = dataSource.rawHierarchyInfo.appInfo;
        ...
        [[RACSignal merge:@[[RACSignal return:nil],
            ...
            @strongify(self);
            NSArray<PickViewDisplayItem *> *validItems = [dataSource.flatItems pickview_filter:^BOOL(PickViewDisplayItem *obj) {
        ...
        [RACObserve(dataSource, selectedItem) subscribeNext:^(PickViewDisplayItem *item) {
            @strongify(self);
...
- (NSView *)makeContainerView {
    ...
    self.stageView.didChangeAppearanceBlock = ^(LKBaseView *view, BOOL isDarkMode) {
        view.backgroundColor = isDarkMode ? PickViewColorMake(0, 0, 0) : PickViewColorMake(255, 255, 255);
    };
...
- (void)viewDidLayout {
    ...
    BOOL hasLayouted = [self pickview_getBindBOOLForKey:@"hasLayouted"];
    if (!hasLayouted) {
        [self pickview_bindBOOL:YES forKey:@"hasLayouted"];
...
- (BOOL)gestureRecognizer:(NSGestureRecognizer *)gestureRecognizer shouldAttemptToRecognizeWithEvent:(NSEvent *)event {
    ...
    if (gestureRecognizer == self.panRecognizer) {
        if (self.previewView.dimension != PickViewPreviewDimension3D && !self.isKeyingDownSpace) {
...
- (void)previewStageView:(LKPreviewStageView *)view mouseMoved:(NSEvent *)event {
    ...
    PickViewDisplayItem *item = [self.previewView displayItemAtPoint:point];
...
- (void)_handleShowHiddenItemsChange:(PickViewMsgActionParams *)param {
    self.previewView.showHiddenItems = param.boolValue;
    [self.previewView updateZPosition];
}

- (void)_handleManagerFreeRotationDidChange:(PickViewMsgActionParams *)params {
    BOOL canFreeRotate = params.boolValue;
    if (canFreeRotate) {
        if (self.previewView.dimension == PickViewPreviewDimension3D) {
...
- (void)_handleManagerPreviewScaleDidChange:(PickViewMsgActionParams *)params {
    double scale = params.doubleValue;
    self.previewView.scale = scale;
}

- (void)_handleManagerPreviewDimensionDidChange:(PickViewMsgActionParams *)params {
    PickViewPreviewDimension newDimension = params.integerValue;
    
    if (self.previewView.dimension == newDimension) {
        return;
    }
    
    if (newDimension == PickViewPreviewDimension2D) {
        [self pickview_bindPoint:self.previewView.rotation forKey:@"prevRotation"];
        [self.previewView setDimension:newDimension animated:YES];
    } else {
        CGPoint rotation = [self pickview_getBindPointForKey:@"prevRotation"];
        [self.previewView setDimension:newDimension animated:YES];
...
- (void)_handleManagerZInterspaceDidChange:(PickViewMsgActionParams *)params {
...
- (void)_handleDoubleClick:(NSClickGestureRecognizer *)recognizer {
    ...
    PVDoubleClickBehavior behavior = [[LKPreferenceManager mainManager] doubleClickBehavior];
...
- (void)scrollWheel:(NSEvent *)event {
    ...
    if (self.isKeyingDownCommand) {
        ...
        targetScale = MAX(MIN(targetScale, PickViewPreviewMaxScale), PickViewPreviewMinScale);
        [manager.previewScale setDoubleValue:targetScale ignoreSubscriber:nil];
...
- (void)_hierarchyInfoDidChange {
    PickViewHierarchyInfo *currentInfo = self.dataSource.rawHierarchyInfo;
    if (!currentInfo) {
        NSAssert(NO, @"");
        return;
    }
    ...
    PickViewHierarchyInfo *prevInfo = [self pickview_getBindObjectForKey:@"prevRawHierarchyInfo"];
    [self pickview_bindObject:currentInfo forKey:@"prevRawHierarchyInfo"];
    if (!prevInfo) {
        return;
    }
...
- (void)menuNeedsUpdate:(NSMenu *)menu {
    ...
    PickViewDisplayItem *displayItem = self.rightClickingDisplayItem;
    if (!displayItem) {
        return;
    }
...
- (void)_handlePrintItem:(NSMenuItem *)menuItem {
    PickViewDisplayItem *item = self.rightClickingDisplayItem;
