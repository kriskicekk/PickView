...
- (void)reloadWithHierarchyInfo:(PickViewHierarchyInfo *)info keepState:(BOOL)keepState {
    ...
    CGFloat maxLengthInPx = PickViewNodeImageMaxLengthInPx - 100;
...
- (void)modifyWithDisplayItemDetail:(PickViewDisplayItemDetail *)detail {
    ...
    PickViewDisplayItem *displayItem = [self displayItemWithOid:detail.displayItemOid];
    if (!displayItem) {
        NSAssert(NO, @"");
        return;
    }
    ...
    if (detail.subitems && (displayItem.subitems.count > 0 || detail.subitems.count > 0)) {
        ...
        self.rawFlatItems = [PickViewDisplayItem flatItemsFromHierarchicalItems:self.rawHierarchyInfo.displayItems];
        self.flatItems = self.rawFlatItems;
        [self.didReloadHierarchyInfo sendNext:nil];
        
        [displayItem enumerateSelfAndChildren:^(PickViewDisplayItem * _Nonnull obj) {
            if (obj == displayItem) {
                return;
            }
...
- (void)_modifyDisplayItem:(PickViewDisplayItem *)item newFrame:(CGRect)frame newBounds:(CGRect)bounds {
    if (!item) {
        NSAssert(NO, @"");
        return;
    }
