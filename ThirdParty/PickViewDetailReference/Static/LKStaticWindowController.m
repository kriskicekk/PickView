...
#import "PickViewHierarchyFile.h"
    ...
    [[[[LKAppsManager sharedInstance] fetchAppInfosWithImage:YES localInfos:nil] deliverOnMainThread] subscribeNext:^(NSArray<LKInspectableApp *> *apps) {
        ...
        vc.didSelectApp = ^(LKInspectableApp *app) {
            ...
            if (app.serverVersionError) {
                if (app.serverVersionError.code == PickViewErrCode_ServerVersionTooLow) {
                    [LKHelper openPickViewWebsiteWithPath:@"faq/server-version-too-low/"];
                } else {
                    [LKHelper openPickViewWebsiteWithPath:@"faq/server-version-too-high/"];
                }
                ...
                [[app fetchHierarchyData] subscribeNext:^(PickViewHierarchyInfo *info) {
...
- (void)appMenuManagerDidSelectReload {    
    ...
    if (self.isFetchingDetails) {
        NSError *error = PickViewErrorMake(NSLocalizedString(@"Cannot reload at this time", NIL), NSLocalizedString(@"Please wait until current sync is completed. You can get sync progress in the upper-left corner of this window.", nil));
        [[NSAlert alertWithError:error] beginSheetModalForWindow:self.window completionHandler:nil];
        return;
    }
    [self _handleReload];
...
- (void)appMenuManagerDidSelectDimension {
    ...
    if (manager.previewDimension.currentIntegerValue == PickViewPreviewDimension2D) {
        [manager.previewDimension setIntegerValue:PickViewPreviewDimension3D ignoreSubscriber:nil];
    } else {
        [manager.previewDimension setIntegerValue:PickViewPreviewDimension2D ignoreSubscriber:nil];
    }
}
...
- (void)appMenuManagerDidSelectZoomIn {
    ...
    double targetScale = MIN(MAX(currentScale + 0.1, PickViewPreviewMinScale), PickViewPreviewMaxScale);
...
- (void)appMenuManagerDidSelectDecreaseInterspace {
    ...
    newValue = MIN(MAX(newValue, PickViewPreviewMinZInterspace), PickViewPreviewMaxZInterspace);
...
- (void)appMenuManagerDidSelectExport {
    ...
    PickViewHierarchyInfo *hierarchyInfo = [LKStaticHierarchyDataSource sharedInstance].rawHierarchyInfo;
    ...
    [panel beginSheetModalForWindow:self.window completionHandler:^(NSModalResponse result) {
        if (result == NSModalResponseOK) {
            ...
            NSError *writeError;
            if (!exportedData) {
                NSAssert(NO, @"PickViewClient - write fail, no data");
                return;
            }
            BOOL writeSucc = [exportedData writeToFile:path options:0 error:&writeError];
            if (!writeSucc) {
                NSLog(@"PickViewClient - write fail:%@", writeError);
            }
        }
    }];
...
- (void)appMenuManagerDidSelectOpenInNewWindow {
    PickViewHierarchyInfo *newHierarchyInfo = [LKStaticHierarchyDataSource sharedInstance].rawHierarchyInfo.copy;
    PickViewHierarchyFile *file = [PickViewHierarchyFile new];
