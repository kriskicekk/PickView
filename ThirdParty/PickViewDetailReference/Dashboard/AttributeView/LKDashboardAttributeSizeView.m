...
- (instancetype)initWithFrame:(NSRect)frameRect {
    if (self = [super initWithFrame:frameRect]) {
        ...
        self.inputsView = [NSArray pickview_arrayWithCount:2 block:^id(NSUInteger idx) {
...
- (void)renderWithAttribute {
    ...
    NSArray<NSString *> *mainStrs = @[[NSString pickview_stringFromDouble:size.width decimal:3],
                                      [NSString pickview_stringFromDouble:size.height decimal:3]];
...
- (void)controlTextDidEndEditing:(NSNotification *)notification {
    ...
    NSUInteger editingTextFieldIdx = [[self.inputsView pickview_map:^id(NSUInteger idx, LKNumberInputView *value) {
