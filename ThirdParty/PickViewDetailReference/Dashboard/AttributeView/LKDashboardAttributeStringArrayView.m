...
- (void)layout {
    ...
    [self.labels enumerateObjectsUsingBlock:^(LKLabel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (idx > 0) {
            if ([self.sepLayers pickview_hasIndex:(idx - 1)]) {
...
- (NSSize)sizeThatFits:(NSSize)limitedSize {
    limitedSize.height = [self.labels pickview_reduceCGFloat:^CGFloat(CGFloat accumulator, NSUInteger idx, LKLabel *obj) {
...
- (void)renderWithAttribute {
    ...
    self.labels = [self.labels pickview_resizeWithCount:lists.count add:^LKLabel *(NSUInteger idx) {
    ...
    if (lists.count > 1) {
        self.sepLayers = [self.sepLayers pickview_resizeWithCount:(lists.count - 1) add:^CALayer *(NSUInteger idx) {
            CALayer *layer = [CALayer new];
            [layer pickview_removeImplicitAnimations];
