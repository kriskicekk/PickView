...
- (void)renderWithAttribute {
    ...
    NSArray<PickViewAutoLayoutConstraint *> *rawData = self.attribute.value;
    NSArray<PickViewAutoLayoutConstraint *> *sortedRawData = [self _sortedRawDataFromData:rawData];
    
    [self.textControls pickview_dequeueWithCount:sortedRawData.count add:^LKDashboardAttributeConstraintsItemControl *(NSUInteger idx) {
...
- (NSSize)sizeThatFits:(NSSize)limitedSize {
    NSArray<LKTextControl *> *visibleControls = [self.textControls pickview_filter:^BOOL(LKTextControl *obj) {
        return !obj.hidden;
    }];
    limitedSize.height = [visibleControls pickview_reduceCGFloat:^CGFloat(CGFloat accumulator, NSUInteger idx, LKTextControl *obj) {
...
- (NSArray<PickViewAutoLayoutConstraint *> *)_sortedRawDataFromData:(NSArray<PickViewAutoLayoutConstraint *> *)rawData {
    return [rawData sortedArrayUsingComparator:^NSComparisonResult(PickViewAutoLayoutConstraint *obj1, PickViewAutoLayoutConstraint *obj2) {
...
- (void)_handleClickItem:(LKDashboardAttributeConstraintsItemControl *)control {
    PickViewAutoLayoutConstraint *constraint = control.constraint;
    ...
    @weakify(popover);
    vc.requestJumpingToObject = ^(PickViewObject *pickviewObj) {
        @strongify(popover);
        [popover close];
        
        LKHierarchyDataSource *dataSource = [self.dashboardViewController currentDataSource];
        PickViewDisplayItem *item = [dataSource displayItemWithOid:pickviewObj.oid];
