//
//  LKDashboardAttributeInsetsView.m
//  PickView
//
...
- (instancetype)initWithFrame:(NSRect)frameRect {
    if (self = [super initWithFrame:frameRect]) {
        ...
        self.mainInputsView = [NSArray pickview_arrayWithCount:4 block:^id(NSUInteger idx) {
...
- (void)renderWithAttribute {
    ...
    NSArray<NSString *> *mainStrs = @[[NSString pickview_stringFromDouble:insets.top decimal:3],
                                  [NSString pickview_stringFromDouble:insets.left decimal:3],
                                  [NSString pickview_stringFromDouble:insets.bottom decimal:3],
                                  [NSString pickview_stringFromDouble:insets.right decimal:3]];
...
- (void)controlTextDidEndEditing:(NSNotification *)notification {
    ...
    NSNumber *inputValue = [LKNumberInputView parsedValueWithString:editingTextField.stringValue attrType:PVAttrTypeDouble];
    if (inputValue == nil) {
        NSLog(@"输入格式校验不通过，驳回");
    ...
    NSUInteger editingTextFieldIdx = [[self.mainInputsView pickview_map:^id(NSUInteger idx, LKNumberInputView *value) {
