...
- (NSArray<NSString *> *)stringListWithAttribute:(PVAttribute *)attribute {
    NSArray<NSString *> *cache = [attribute pickview_getBindObjectForKey:@"cachedDemangled"];
    if (cache) {
        return cache;
    }
    NSArray<NSString *> *result = attribute.value;
    NSArray<NSString *> *demangled = [result pickview_map:^id(NSUInteger idx, NSString *raw) {
        return [self demangle:raw];
    }];
    [attribute pickview_bindObject:demangled forKey:@"cachedDemangled"];
    return demangled;
}
