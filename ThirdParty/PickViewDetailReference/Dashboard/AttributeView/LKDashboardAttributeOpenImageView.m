...
- (instancetype)initWithFrame:(NSRect)frameRect {
    if (self = [super initWithFrame:frameRect]) {
        ...
        self.borderColors = LKColorsCombine(PickViewColorMake(181, 181, 181), PickViewColorMake(83, 83, 83));
...
- (void)_handleClick {
    ...
    if (imageViewOid_num == nil) {
        AlertError(PickViewErr_Inner, self.window);
        NSAssert(NO, @"");
        return;
    }
    ...
    if (!InspectingApp) {
        AlertError(PickViewErr_NoConnect, self.window);
        return;
    }
