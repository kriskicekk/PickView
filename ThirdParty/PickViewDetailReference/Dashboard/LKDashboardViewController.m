...
#import "PickViewCustomAttrModification.h"
...
- (void)_didInitialized {
    ...
    if (self.staticDataSource) {
        ...
        [[self.staticDataSource.itemDidChangeAttrGroup deliverOnMainThread] subscribeNext:^(PickViewDisplayItem *displayItem) {
            @strongify(self);
...
- (RACSignal *)modifyCustomAttribute:(PVAttribute *)attribute newValue:(id)newValue {
    ...
    return [RACSignal createSignal:^RACDisposable * _Nullable(id<RACSubscriber>  _Nonnull subscriber) {
        @strongify(self);        
        PickViewCustomAttrModification *modification = [PickViewCustomAttrModification new];
        ...
        if (modification.customSetterID.length == 0) {
            NSAssert(NO, @"");
            AlertError(PickViewErr_Inner, self.view.window);
            [subscriber sendError:PickViewErr_Inner];
            return nil;
        }
        
        if (![LKAppsManager sharedInstance].inspectingApp) {
            AlertError(PickViewErr_NoConnect, self.view.window);
            [subscriber sendError:PickViewErr_NoConnect];
            return nil;
        }
...
- (RACSignal *)modifyInbuiltAttribute:(PVAttribute *)attribute newValue:(id)newValue {
    ...
    return [RACSignal createSignal:^RACDisposable * _Nullable(id<RACSubscriber>  _Nonnull subscriber) {
        @strongify(self);
        PickViewDisplayItem *modifyingItem = attribute.targetDisplayItem;
        
        PVAttributeModification *modification = [PVAttributeModification new];
        modification.clientReadableVersion = [LKHelper pickviewReadableVersion];
        if ([PickViewDashboardBlueprint isUIViewPropertyWithAttrID:attribute.identifier]) {
            modification.targetOid = modifyingItem.viewObject.oid;
        } else {
            modification.targetOid = modifyingItem.layerObject.oid;
        }
        modification.setterSelector = [PickViewDashboardBlueprint setterWithAttrID:attribute.identifier];
        modification.attrType = attribute.attrType;
        modification.value = newValue;
        ...
        @weakify(self);
        [[[LKAppsManager sharedInstance].inspectingApp submitInbuiltModification:modification] subscribeNext:^(PickViewDisplayItemDetail *detail) {
            ...
            if (self.staticDataSource) {
                ...
                if ([PickViewDashboardBlueprint needPatchAfterModificationWithAttrID:attribute.identifier]) {
                    [[LKStaticAsyncUpdateManager sharedInstance] updateAfterModifyingDisplayItem:(PickViewStaticDisplayItem *)modifyingItem];
                }
...
- (void)dashboardCardViewNeedToggleCollapse:(LKDashboardCardView *)view {
    ...
    if ([manager.collapsedAttrGroups containsObject:view.attrGroup.identifier]) {
        view.isCollapsed = NO;
        manager.collapsedAttrGroups = [manager.collapsedAttrGroups pickview_arrayByRemovingObject:view.attrGroup.identifier];
    } else {
...
- (void)dashboardHeaderView:(LKDashboardHeaderView *)view didInputString:(NSString *)searchString {
        ...
        [group.attrSections enumerateObjectsUsingBlock:^(PVAttributesSection * _Nonnull section, NSUInteger idx, BOOL * _Nonnull stop) {
            [section.attributes enumerateObjectsUsingBlock:^(PVAttribute * _Nonnull attr, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                if (attr.isUserCustom) {
                    ...
                    title = [PickViewDashboardBlueprint fullTitleWithAttrID:attr.identifier];
                }
    ...
    [self.searchPropViews pickview_dequeueWithCount:resultAttrs.count add:^LKDashboardSearchPropView *(NSUInteger idx) {
    ...
    PickViewObject *selectedObj = self.currentDataSource.selectedItem.viewObject ? : self.currentDataSource.selectedItem.layerObject;
...
- (void)dashboardSearchMethodsView:(LKDashboardSearchMethodsView *)view requestToInvokeMethod:(NSString *)method oid:(unsigned long)oid {
    ...
    if (oid == 0 || method.length == 0) {
        signal = [RACSignal error:PickViewErr_Inner];
    } else if (![LKAppsManager sharedInstance].inspectingApp) {
        signal = [RACSignal error:PickViewErr_NoConnect];
    } else {
...
- (void)dashboardSearchPropView:(LKDashboardSearchPropView *)view didClickRevealAttribute:(PVAttribute *)clickedAttr {
