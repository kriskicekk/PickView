...
- (NSSize)sizeThatFits:(NSSize)limitedSize {
    ...
    if (self.titleLabel.isVisible) {
        ...
        limitedSize.height = [self.itemViews pickview_reduceCGFloat:^CGFloat(CGFloat accumulator, NSUInteger idx, LKTextControl *view) {
...
- (void)renderWithMethods:(NSArray<NSString *> *)methods oid:(unsigned long)oid {
    ...
    [self.itemViews pickview_dequeueWithCount:methods.count add:^LKTextControl *(NSUInteger idx) {
        ...
        view.hidden = NO;
        [view pickview_bindObject:methods[idx] forKey:@"methodName"];
...
- (void)_handleMethodControl:(NSControl *)control {
    NSString *methodName = [control pickview_getBindObjectForKey:@"methodName"];
