//
//  PVDetailDashboardAttributeRelationView.m
//  PickViewMac
//
...
@implementation PVDetailDashboardAttributeRelationView

- (NSArray<NSString *> *)stringListWithAttribute:(PVAttribute *)attribute {
    NSArray<NSString *> *cache = [attribute pv_inspect_getBindObjectForKey:@"cachedDemangled"];
    if (cache) {
        return cache;
    }
    NSArray<NSString *> *result = attribute.value;
    NSArray<NSString *> *demangled = [result pv_inspect_map:^id(NSUInteger idx, NSString *raw) {
        return [self demangle:raw];
    }];
    [attribute pv_inspect_bindObject:demangled forKey:@"cachedDemangled"];
    return demangled;
}
...
- (NSString *)demangle:(NSString *)rawText {
    {
        ...
        if (match) {
            ...
            NSString *demangled1 = [PVDetailSwiftDemangler simpleParseWithInput:substring1];
            NSString *demangled2 = [PVDetailSwiftDemangler simpleParseWithInput:substring2];
    ...
    
    {
        ...
        if (match) {
            ...
            NSString *demangledText = [PVDetailSwiftDemangler simpleParseWithInput:rawClassName];
