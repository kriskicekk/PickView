...
@implementation PVDetailTipsView {
    CGFloat _insetLeft;
...
- (void)updateColors {
    [super updateColors];
    self.backgroundColor = self.isDarkMode ? PVColorRGBAMake(0, 0, 0, .8) : PVColorRGBAMake(255, 255, 255, .9);
    self.titleLabel.textColor = self.isDarkMode ? PVColorMake(197, 198, 199) : PVColorMake(108, 109, 110);
    
    NSColor *borderColor = self.isDarkMode ? PVColorMake(43, 44, 45) : PVColorMake(216, 217, 218);
...
- (NSColor *)buttonTextColor {
    return self.isDarkMode ? PVColorMake(64, 134, 216) : PVColorMake(74, 145, 228);
}

@end

@interface PVDetailYellowTipsView ()

@property(nonatomic, assign) BOOL isAnimating;

@end

@implementation PVDetailYellowTipsView
...
- (void)updateColors {
    ...
    self.sepLayer.backgroundColor = PVColorRGBAMake(255, 255, 255, .5).CGColor;
}
...
@implementation PVDetailRedTipsView

- (void)startAnimation {
    CABasicAnimation *anim = [CABasicAnimation animationWithKeyPath:@"backgroundColor"];
    anim.fromValue = (id)PVColorRGBAMake(208, 2, 27, .9).CGColor;
    anim.toValue = (id)PVColorRGBAMake(208, 2, 27, .7).CGColor;
    anim.duration = .8;
    anim.repeatCount = HUGE_VALF;
    anim.autoreverses = YES;
    [self.layer removeAllAnimations];
