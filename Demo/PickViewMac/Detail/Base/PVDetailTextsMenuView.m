...
@interface PVDetailTextsMenuView ()

@property(nonatomic, strong) NSMutableArray<PVDetailLabel *> *leftLabels;
@property(nonatomic, strong) NSMutableArray<PVDetailLabel *> *rightLabels;

@property(nonatomic, strong) NSMutableDictionary<NSNumber *, NSButton *> *buttons;

@end

@implementation PVDetailTextsMenuView {
...
- (void)layout {
    ...
    NSArray<PVDetailLabel *> *visibleRightLabels = self.rightLabels.lk_visibleViews;
    NSArray<PVDetailLabel *> *visibleLeftLabels = self.leftLabels.lk_visibleViews;
    
    if (self.type == PVDetailTextsMenuViewTypeCenter) {
        __block CGFloat leftLabelMaxWidth = self.insets.left;
        [visibleLeftLabels enumerateObjectsUsingBlock:^(PVDetailLabel * _Nonnull leftLabel, NSUInteger idx, BOOL * _Nonnull stop) {
            PVDetailLabel *prevLeftLabel = (idx > 0 ? visibleLeftLabels[idx - 1] : nil);
        ...
        [visibleLeftLabels enumerateObjectsUsingBlock:^(PVDetailLabel * _Nonnull leftLabel, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            PVDetailLabel *rightLabel = visibleRightLabels[idx];
        ...
        [visibleRightLabels enumerateObjectsUsingBlock:^(PVDetailLabel * _Nonnull rightLabel, NSUInteger idx, BOOL * _Nonnull stop) {
            PVDetailLabel *prevLeftLabel = (idx > 0 ? visibleLeftLabels[idx - 1] : nil);
            
            PVDetailLabel *leftLabel = visibleLeftLabels[idx];
            CGFloat y = prevLeftLabel ? (prevLeftLabel.$maxY + self.verSpace) : 0;
...
- (void)setTexts:(NSArray<PVStringTwoTuple *> *)texts {
    _texts = texts.copy;
    
    [self.leftLabels pv_inspect_dequeueWithCount:texts.count add:^PVDetailLabel *(NSUInteger idx) {
        PVDetailLabel *label = [PVDetailLabel new];
        ...
        return label;
    } notDequeued:^(NSUInteger idx, PVDetailLabel *obj) {
        obj.hidden = YES;
    } doNext:^(NSUInteger idx, PVDetailLabel *obj) {
        obj.hidden = NO;
        obj.stringValue = texts[idx].first;
    }];

    [self.rightLabels pv_inspect_dequeueWithCount:texts.count add:^PVDetailLabel *(NSUInteger idx) {
        PVDetailLabel *label = [PVDetailLabel new];
        label.selectable = YES;
        label.font = self.font;
        label.maximumNumberOfLines = 1;
...
- (void)setType:(PVDetailTextsMenuViewType)type {
    _type = type;
    [self _updateAlignments];
}

- (void)_updateAlignments {
    [self.leftLabels enumerateObjectsUsingBlock:^(PVDetailLabel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (self.type == PVDetailTextsMenuViewTypeJustified) {
            obj.alignment = NSTextAlignmentLeft;
        } else {
            obj.alignment = NSTextAlignmentRight;
        }
    }];
    [self.rightLabels enumerateObjectsUsingBlock:^(PVDetailLabel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (self.type == PVDetailTextsMenuViewTypeJustified) {
            obj.alignment = NSTextAlignmentRight;
        } else {
            obj.alignment = NSTextAlignmentLeft;
...
- (NSSize)sizeThatFits:(NSSize)limitedSize {
    ...
    [self.leftLabels.lk_visibleViews enumerateObjectsUsingBlock:^(PVDetailLabel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    [self.rightLabels.lk_visibleViews enumerateObjectsUsingBlock:^(PVDetailLabel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
