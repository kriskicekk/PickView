...
#import "ShortCocoa+PVClient.h"
#import <objc/runtime.h>
#import "ShortCocoa+Private.h"
#import "PVDetailBaseView.h"

@implementation ShortCocoa (PickView)

+ (void)load {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        Method oriMethod = class_getInstanceMethod([self class], @selector(sizeToFit));
        Method newMethod = class_getInstanceMethod([self class], @selector(pv_inspect_sizeToFit));
        method_exchangeImplementations(oriMethod, newMethod);
        
        Method oriMethod1 = class_getInstanceMethod([self class], @selector(heightToFit));
        Method newMethod1 = class_getInstanceMethod([self class], @selector(pv_inspect_heightToFit));
        method_exchangeImplementations(oriMethod1, newMethod1);
    });
}

- (ShortCocoa *)pv_inspect_sizeToFit {
    [self unpackClassA:[PVDetailBaseView class] doA:^(PVDetailBaseView * _Nonnull view, BOOL * _Nonnull stop) {
...
- (ShortCocoa *)pv_inspect_heightToFit {
    [self unpackClassA:[PVDetailBaseView class] doA:^(PVDetailBaseView * _Nonnull view, BOOL * _Nonnull stop) {
