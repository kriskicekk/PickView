...
@implementation PVDetailHelper

+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    static PVDetailHelper *instance = nil;
...
+ (void)openPickViewWebsiteWithPath:(NSString *)path {
    NSString *version = [[self pickviewReadableVersion] stringByReplacingOccurrencesOfString:@"." withString:@"d"];
    NSString *urlString = [NSString stringWithFormat:@"https://pickview/%@?v=%@", path, version];
    [[NSWorkspace sharedWorkspace] openURL:[NSURL URLWithString:urlString]];
}

+ (void)openPickViewOfficialWebsite {
    [[NSWorkspace sharedWorkspace] openURL:[NSURL URLWithString:@"https://pickview"]];
}

+ (void)openCustomConfigWebsite {
    [[NSWorkspace sharedWorkspace] openURL:[NSURL URLWithString:@"https://pickview/faq/config-file/"]];
}
...
+ (NSArray<NSString *> *)bestMatchesInCandidates:(NSArray<NSString *> *)candidates input:(NSString *)input maxResultsCount:(NSUInteger)maxResultsCount {
    ...
    [candidates enumerateObjectsUsingBlock:^(NSString * _Nonnull candidate, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (score > lowestScore) {
            NSUInteger idxToDelete = [self _indexOfSmallestNumberInArray:[topResults pv_inspect_map:^id(NSUInteger idx, NSDictionary *value) {
                return value[@"score"];
            }]];
            [topResults removeObjectAtIndex:idxToDelete];
            [topResults addObject:@{@"string": candidate, @"score":@(score)}];
            lowestScore = [self _smallestNumberInArray:[topResults pv_inspect_map:^id(NSUInteger idx, NSDictionary *value) {
                return value[@"score"];
            }]];
        }
    }];
    ...
    NSArray<NSString *> *resultStrings = [topResults pv_inspect_map:^id(NSUInteger idx, NSDictionary *value) {
        return value[@"string"];
    }];
