...
@interface PVDetailLaunchAppView ()
...
@property(nonatomic, strong) PVDetailLabel *errorTitleLabel;
@property(nonatomic, strong) PVDetailLabel *errorSubtitleLabel;

@end

@implementation PVDetailLaunchAppView {
    NSSize _previewSize;
...
- (void)setApp:(PVDetailInspectableApp *)app {
    _app = app;
    ...
    if (app.serverVersionError) {
        ...
        if (app.serverVersionError.code == PVInspectErrCode_ServerVersionTooLow) {
            self.errorTitleLabel.stringValue = NSLocalizedString(@"The version of PickViewServer linked with this iOS App is too low.", nil);
            
        } else if (app.serverVersionError.code == PVInspectErrCode_ServerVersionTooHigh) {
            self.errorTitleLabel.stringValue = NSLocalizedString(@"Unable to inspect this iOS App. Current version of PickView app is too low.", nil);
        ...
        if (app.appInfo.deviceType == PVAppInfoDeviceSimulator) {
...
- (void)updateLayer {
    [super updateLayer];
    self.hoverBgLayer.backgroundColor = self.effectiveAppearance.lk_isDarkMode ? PVColorRGBAMake(0, 0, 0, .17).CGColor : PVColorRGBAMake(0, 0, 0, .08).CGColor;
    
    if (self.app.serverVersionError) {
        self.layer.backgroundColor = self.effectiveAppearance.lk_isDarkMode ? PVColorRGBAMake(0, 0, 0, .13).CGColor : PVColorRGBAMake(0, 0, 0, .05).CGColor;
    } else {
...
- (void)_initErrorViewsIfNeeded {
    ...
    if (!self.errorTitleLabel) {
        self.errorTitleLabel = [PVDetailLabel new];
    ...
    if (!self.errorSubtitleLabel) {
        self.errorSubtitleLabel = [PVDetailLabel new];
