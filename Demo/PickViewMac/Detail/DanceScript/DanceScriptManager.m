...
- (NSString *)copyScriptFromAppToDisk {
    NSString *newScriptPath = [NSTemporaryDirectory() stringByAppendingPathComponent:@"PickView_DanceJump.sh"];
    //    NSString *desktopPath = [NSSearchPathForDirectoriesInDomains(NSDesktopDirectory, NSUserDomainMask, YES) firstObject];
    //    NSString *newScriptPath = [desktopPath stringByAppendingPathComponent:@"PickView_DanceJump.sh"];
...
/*
 ...
 对应 pv_inspect_source 的类型是这个 json 结构
...
- (void)executeWithType:(NSString *)type method:(NSString *)method path:(NSString *)buildPath {
    ...
    [PVDetailAnalytics trackEvent:@"DanceJump"];
}
