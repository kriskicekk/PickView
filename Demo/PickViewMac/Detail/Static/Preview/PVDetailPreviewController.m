...
#import "PVDetailPreviewPanGestureRecognizer.h"
...
extern NSString *const PVDetailAppShowConsoleNotificationName;

@interface PVDetailPreviewController () <NSGestureRecognizerDelegate, PVDetailPreviewStageViewDelegate, NSMenuDelegate>

@property(nonatomic, strong) NSClickGestureRecognizer *clickRecognizer;

@property(nonatomic, strong) PVDetailPreviewPanGestureRecognizer *panRecognizer;

@property(nonatomic, strong) PVDetailHierarchyDataSource *dataSource;

@property(nonatomic, strong) PVDetailPreviewStageView *stageView;
@property(nonatomic, strong) PVDetailPreviewView *previewView;
...
@property(nonatomic, strong) PVDisplayItem *rightClickingDisplayItem;
...
@implementation PVDetailPreviewController

- (instancetype)initWithDataSource:(PVDetailHierarchyDataSource *)dataSource {
    if (self = [self initWithContainerView:nil]) {
        self.dataSource = dataSource;
        
        PVAppInfo *appInfo = dataSource.rawHierarchyInfo.appInfo;
        
        self.previewView = [[PVDetailPreviewView alloc] initWithDataSource:dataSource];
        ...
        @weakify(self);
        self.stageView.didChangeAppearanceBlock = ^(PVDetailBaseView *view, BOOL isDarkMode) {
            @strongify(self);
        ...
        [[RACSignal merge:@[[RACSignal return:nil],
            ...
            @strongify(self);
            NSArray<PVDisplayItem *> *validItems = [dataSource.flatItems pv_inspect_filter:^BOOL(PVDisplayItem *obj) {
        ...
        if ([dataSource isKindOfClass:[PVDetailStaticHierarchyDataSource class]]) {
            [signalsToUpdateZIndex addObject:((PVDetailStaticHierarchyDataSource *)dataSource).itemDidChangeHiddenAlphaValue];
            [signalsToUpdateZIndex addObject:((PVDetailStaticHierarchyDataSource *)dataSource).itemsDidChangeFrame];
        }
        ...
        self.panRecognizer = [[PVDetailPreviewPanGestureRecognizer alloc] initWithTarget:self action:@selector(_handlePanGesture:)];
...
- (NSView *)makeContainerView {
    self.stageView = [[PVDetailPreviewStageView alloc] init];
    self.stageView.didChangeAppearanceBlock = ^(PVDetailBaseView *view, BOOL isDarkMode) {
        view.backgroundColor = isDarkMode ? PVColorMake(0, 0, 0) : PVColorMake(255, 255, 255);
    };
...
- (void)viewDidLayout {
    ...
    CGFloat titleBarHeight = [PVDetailNavigationManager sharedInstance].windowTitleBarHeight;
    ...
    BOOL hasLayouted = [self pv_inspect_getBindBOOLForKey:@"hasLayouted"];
    if (!hasLayouted) {
        [self pv_inspect_bindBOOL:YES forKey:@"hasLayouted"];
...
- (BOOL)gestureRecognizer:(NSGestureRecognizer *)gestureRecognizer shouldAttemptToRecognizeWithEvent:(NSEvent *)event {
    ...
    if (gestureRecognizer == self.panRecognizer) {
        if (self.previewView.dimension != PVPreviewDimension3D && !self.isKeyingDownSpace) {
...
#pragma mark - <PVDetailPreviewStageViewDelegate>
...
- (void)previewStageView:(PVDetailPreviewStageView *)view mouseMoved:(NSEvent *)event {
    ...
    PVDisplayItem *item = [self.previewView displayItemAtPoint:point];
...
- (void)_handleManagerFreeRotationDidChange:(PVDetailMsgActionParams *)params {
    BOOL canFreeRotate = params.boolValue;
    if (canFreeRotate) {
        if (self.previewView.dimension == PVPreviewDimension3D) {
...
- (void)_handleManagerPreviewScaleDidChange:(PVDetailMsgActionParams *)params {
    double scale = params.doubleValue;
    self.previewView.scale = scale;
}

- (void)_handleManagerPreviewDimensionDidChange:(PVDetailMsgActionParams *)params {
    PVPreviewDimension newDimension = params.integerValue;
    
    if (self.previewView.dimension == newDimension) {
        return;
    }
    
    if (newDimension == PVPreviewDimension2D) {
        [self pv_inspect_bindPoint:self.previewView.rotation forKey:@"prevRotation"];
        [self.previewView setDimension:newDimension animated:YES];
    } else {
        CGPoint rotation = [self pv_inspect_getBindPointForKey:@"prevRotation"];
        [self.previewView setDimension:newDimension animated:YES];
...
- (void)_handleManagerZInterspaceDidChange:(PVDetailMsgActionParams *)params {
    double doubleValue = params.doubleValue;
    self.previewView.zInterspace = doubleValue;
}

- (void)_handlePanGesture:(PVDetailPreviewPanGestureRecognizer *)recognizer {
    if (recognizer.state == NSGestureRecognizerStateBegan) {
        [[PVDetailUserActionManager sharedInstance] sendAction:PVDetailUserActionType_PreviewOperation];
...
- (void)magnifyWithEvent:(NSEvent *)event {
    ...
    if (event.phase == NSEventPhaseChanged) {
        PVDetailPreferenceManager *manager = self.dataSource.preferenceManager;
...
- (void)scrollWheel:(NSEvent *)event {
    ...
    if (self.isKeyingDownCommand) {
        ...
        targetScale = MAX(MIN(targetScale, PVPreviewMaxScale), PVPreviewMinScale);
        [manager.previewScale setDoubleValue:targetScale ignoreSubscriber:nil];
...
- (void)setIsKeyingDownOption:(BOOL)isKeyingDownOption {
    ...
    if (self.dataSource.preferenceManager.measureState.currentIntegerValue == PVMeasureState_locked) {
        return;
    }
    
    PVMeasureState state;
    if (isKeyingDownOption && self.dataSource.selectedItem && [self.view.window isKeyWindow]) {
        state = PVMeasureState_unlocked;
    } else {
        state = PVMeasureState_no;
    }
...
- (void)_hierarchyInfoDidChange {
    PVHierarchyInfo *currentInfo = self.dataSource.rawHierarchyInfo;
    if (!currentInfo) {
        NSAssert(NO, @"");
        return;
    }
    ...
    PVHierarchyInfo *prevInfo = [self pv_inspect_getBindObjectForKey:@"prevRawHierarchyInfo"];
    [self pv_inspect_bindObject:currentInfo forKey:@"prevRawHierarchyInfo"];
    if (!prevInfo) {
        return;
    }
    ...
    [self.dataSource.preferenceManager.previewScale setDoubleValue:PVDetailInitialPreviewScale ignoreSubscriber:nil];
}
...
- (void)menuNeedsUpdate:(NSMenu *)menu {
    ...
    PVDisplayItem *displayItem = self.rightClickingDisplayItem;
    if (!displayItem) {
        return;
    }
    ...
    if (!displayItem.isUserCustom) {
        ...
        if (!self.dataSource.isReadOnly) {
            BOOL isUpdating = [PVDetailStaticAsyncUpdateManager sharedInstance].isUpdating;
            [menu addItem:({
                NSMenuItem *item = [NSMenuItem new];
...
- (void)_handlePrintItem:(NSMenuItem *)menuItem {
    PVDisplayItem *item = self.rightClickingDisplayItem;
    [[NSNotificationCenter defaultCenter] postNotificationName:PVDetailAppShowConsoleNotificationName object:item];
}
...
- (void)_handleReloadSelfItem:(NSMenuItem *)menuItem {
    ...
    [[PVDetailStaticAsyncUpdateManager sharedInstance] reloadSingleDisplayItem:item];
}
...
- (void)_handleReloadSelfAndChildrenItem:(NSMenuItem *)menuItem {
    ...
    [[PVDetailStaticAsyncUpdateManager sharedInstance] reloadDisplayItemAndChildren:item];
}
...
- (void)_handleCancelPreview:(NSMenuItem *)menuItem {
    [PVDetailTutorialManager sharedInstance].togglePreview = YES;
...
- (void)_handleExportScreenshot:(NSMenuItem *)menuItem {
    PVDisplayItem *item = self.rightClickingDisplayItem;
    [PVDetailExportManager exportScreenshotWithDisplayItem:item];
}
