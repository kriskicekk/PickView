...
@interface PVDetailHierarchyRowView () <PVDisplayItemDelegate>
...
@implementation PVDetailHierarchyRowView
...
- (void)_handleClickEventHandlerButton:(NSButton *)button {
    ...
    BOOL editable = NO;
    if (self.window == [PVDetailNavigationManager sharedInstance].staticWindowController.window) {
        editable = YES;
    }
    
    PVDetailHierarchyHandlersPopoverController *vc = [[PVDetailHierarchyHandlersPopoverController alloc] initWithDisplayItem:self.displayItem editable:editable];
    popover.animates = NO;
...
- (void)_updateLabelStringsAndImageViewAlpha {
    ...
    if (self.displayItem.isInSearch && self.displayItem.highlightedSearchString.length) {
        // 搜索状态下，且命中了搜索词
        NSColor *bgColor = self.isDarkMode ? PVColorMake(190, 120, 0) : PVColorMake(255, 240, 100);
...
- (void)displayItem:(PVDisplayItem *)displayItem propertyDidChange:(PVDisplayItemProperty)property {
    if (property == PVDisplayItemProperty_IsHovered) {
        self.isHovered = (self.dataSource.hoveredItem == self.displayItem);
    } else {
        [self reRender];
    }
...
- (void)updateStrikethroughLayer {
    ...
    if (shouldShow) {
        if (!self.strikethroughLayer) {
            ...
            [self.strikethroughLayer pv_inspect_removeImplicitAnimations];
        ...
        if (self.isSelected) {
            ...
            self.strikethroughLayer.backgroundColor = self.isDarkMode ? PVColorRGBAMake(255, 255, 255, .2).CGColor : PVColorRGBAMake(0, 0, 0, .2).CGColor;
        }
...
- (void)_updateEventHandlerButtonColors {
    ...
    if (self.isSelected) {
        ...
        color = PVColorRGBAMake(74, 144, 226, alpha);
    }
...
- (void)_updateLabelsFonts {
    ...
    if (!self.displayItem.isUserCustom && noImage) {
        self.titleLabel.font = [PVDetailHelper italicFontOfSize:13];
        self.subtitleLabel.font = [PVDetailHelper italicFontOfSize:12];
    } else {
...
- (void)updateExpandStatus {
    if (!self.displayItem) {
        self.status = PVDetailOutlineRowViewStatusNotExpandable;
        return;
    }
    if (!self.displayItem.isExpandable) {
        self.status = PVDetailOutlineRowViewStatusNotExpandable;
    } else if (self.displayItem.isExpanded) {
        self.status = PVDetailOutlineRowViewStatusExpanded;
    } else {
        self.status = PVDetailOutlineRowViewStatusCollapsed;
    }
}
...
- (NSImage *)resolveIconImage {
    ...
    if (item.pv_isFlutterItem) {
        ...
        [item.viewObject.classChainList enumerateObjectsUsingBlock:^(NSString * _Nonnull className, NSUInteger idx, BOOL * _Nonnull stop) {
            imageName = [viewsList pv_inspect_firstFiltered:^BOOL(NSDictionary<NSString *, NSString*> *obj) {
