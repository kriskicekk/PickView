...
@interface PVDetailExportAccessoryView ()

@property(nonatomic, assign, readwrite) CGFloat selectedCompression;

@property(nonatomic, strong) PVDetailLabel *compressionLabel;

@property(nonatomic, strong) NSPopUpButton *compressionButton;

@property(nonatomic, strong) PVDetailLabel *sizeLabel;

@property(nonatomic, copy) NSArray<NSNumber *> *compressionArray;

@end

@implementation PVDetailExportAccessoryView {
    CGFloat _insetTop;
...
- (instancetype)initWithFrame:(NSRect)frameRect {
    if (self = [super initWithFrame:frameRect]) {
        ...
        self.compressionLabel = [PVDetailLabel new];
        ...
        [self.compressionButton addItemsWithTitles:[self.compressionArray pv_inspect_map:^id(NSUInteger idx, NSNumber *value) {
            return [NSString stringWithFormat:@"%@%%", @(value.doubleValue * 100)];
        }]];
        [self addSubview:self.compressionButton];
        
        self.sizeLabel = [PVDetailLabel new];
        ...
        @weakify(self);
        [[RACObserve([PVDetailPreferenceManager mainManager], preferredExportCompression) distinctUntilChanged] subscribeNext:^(NSNumber *num) {
            @strongify(self);
...
- (void)_handleCompressionButton {
    ...
    [PVDetailPreferenceManager mainManager].preferredExportCompression = compression;
}
